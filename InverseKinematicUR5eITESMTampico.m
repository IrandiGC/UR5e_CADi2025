function output = InverseKinematicUR5eITESMTampico(H_Actual,q_Anterior)
    ...
    temp1 = sum((P-H_Actual(1:3,4)).^2,1)
    theta = theta(:,temp1 < 0.001);
    %nos quedamos con la solución que esta mas cerca del punto de partida.
    temp2 = sum((theta-q_Anterior).^2,1)
    output = theta(:,temp2 == min(temp2));
